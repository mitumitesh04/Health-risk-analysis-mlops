...
def prediction_interface():
    ...
    with col2:
        ...
        if submitted:
            ...
            try:
                # Call API
                response = requests.post("http://api:8000/predict", json=data, timeout=10)
...
def mlops_pipeline_page():
    ...
    commands = {
        ...
        "Check API Health": "curl http://api:8000/health"
    }
...
def system_monitoring_page():
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            # Get model info
            try:
                model_response = requests.get("http://api:8000/model/info", timeout=5)
